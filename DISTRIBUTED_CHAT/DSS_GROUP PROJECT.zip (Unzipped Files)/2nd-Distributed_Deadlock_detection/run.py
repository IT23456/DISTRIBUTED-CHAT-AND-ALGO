...
import shutil
for i in range(1,users+1):
    shutil.copy("resource_call.py", f"user{i}")
...
for i in range(1,users+1):
    ...
    os.chdir(f"user{i}")
    # start a terminal in each folder and run sender_receiver.py from there
    os.system(f"gnome-terminal -e 'bash -c \"python3 resource_call.py; exec bash\"'")
    os.chdir("..")

os.system(f"gnome-terminal -e 'bash -c \"python3 writer.py; exec bash\"'")
# wait for 5 sec
time.sleep(5)
# run deadlock.cpp 

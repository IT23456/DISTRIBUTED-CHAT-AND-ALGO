...
import shutil
for i in range(1,users+1):
    shutil.copy("text.txt", f"user{i}")
    shutil.copy("sender_receiver.py", f"user{i}")
    shutil.copy("user.py", f"user{i}")
...
for i in range(1,users+1):
    ...
    os.chdir(f"user{i}")
    # start a terminal in each folder and run sender_receiver.py from there
    os.system(f"gnome-terminal -e 'bash -c \"python3 sender_receiver.py; exec bash\"'")
    os.chdir("..")
